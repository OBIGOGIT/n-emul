...
module.exports = {
    ...
    initialize: function () {
        ...
        jQuery("#geo-map-current-location").bind("click", function () {
        ...
        function _getCurrentLocation()
        {
            if (navigator.geolocation) {
                // Use method getCurrentPositionCopy to get coordinates
                navigator.geolocation.getCurrentPosition(_updateMapWithLocation);
            }
            else
                alert('geolocation not supported');
        }

        function _updateMapWithLocation(position)
        {
            var currentLatitude = position.coords.latitude;
            var currentLongitude = position.coords.longitude;
        
            geo.map.setCenter(new OpenLayers.LonLat(currentLongitude, currentLatitude)
                .transform(
                    new OpenLayers.Projection("EPSG:4326"), // transform from WGS 1984
                    new OpenLayers.Projection("EPSG:900913") // to Spherical Mercator Projection
                ),
